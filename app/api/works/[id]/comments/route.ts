...
export async function POST(request: NextRequest, { params }: Context) {
  ...
  if (!content || content.length > 2000) {
    return NextResponse.json({ error: "El comentario debe contener texto y no superar los 2000 caracteres." }, { status: 400 })
  }
  ...
  if (recent) {
    return attachVisitorCookie(
      NextResponse.json({ error: "Espera un minuto antes de comentar de nuevo." }, { status: 429 }),
      actor.newVisitorId,
    )
  }
  ...
  await db.insert(comments).values({
    ...
    visitorName: actor.userId ? null : "Visitante",
    content,
    categories: [],
  })
  ...
  return attachVisitorCookie(
    NextResponse.json({ id: commentId }, { status: 201 }),
    actor.newVisitorId,
  )
}
