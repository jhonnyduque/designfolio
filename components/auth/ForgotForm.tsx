  ...
  if (sent) {
    return (
      <div className="text-center">
        ...
        <h2 className="text-section text-gray-900">Email enviado</h2>
        <p className="mt-2 text-body-sm text-gray-500">
          Si existe una cuenta con <strong>{email}</strong>, recibirás un enlace para restablecer tu contraseña.
        </p>
        <Link href="/login" className="mt-4 inline-block text-action text-gray-900 hover:underline">
          Volver al login
        </Link>
      </div>
    )
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        <div>
          <label htmlFor="resetEmail" className="block text-label text-gray-700">
            Email
          </label>
          <input
            id="resetEmail"
