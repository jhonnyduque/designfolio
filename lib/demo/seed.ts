 ...
 * llevan el prefijo `demo-` y borrarDemo() los quita junto con todo lo que
 * cuelga de ellos.
 *
 * Vive en lib/ y no en scripts/ porque lo usan dos sitios: el CLI
 * (scripts/seed-demo.ts) y el panel de la fundadora. Todo pasa por Drizzle con
 * valores parametrizados; no hay SQL construido a mano en ninguna de las dos
 * direcciones.
 *
 * Los medios salen de `public/demo/`, así que viajan con el despliegue y no
 * dependen de que MEDIA_ROOT tenga nada.
 */
...
const SEMILLA_INICIAL = 20260917
let semilla = SEMILLA_INICIAL
...
export async function crearDemo() {
  ...
  // El servidor no se reinicia entre llamadas como sí hace el CLI, así que la
  // semilla se reinicia aquí para que dos cargas den siempre el mismo conjunto.
  ...
  if (existentes.length > 0) {
    throw new Error(`Ya hay ${existentes.length} perfiles de demostración. Retíralos antes de volver a cargarlos.`)
  }
...
/** Cuánto hay cargado ahora mismo, para que la interfaz sepa qué ofrecer. */
export async function contarDemo(): Promise<{ perfiles: number; obras: number }> {
  const db = getDb()
  const perfilesDemo = await db
    .select({ id: profiles.id })
    .from(profiles)
    .where(like(profiles.username, `${PREFIJO}%`))

  if (perfilesDemo.length === 0) return { perfiles: 0, obras: 0 }

  const obras = await db
    .select({ id: works.id })
    .from(works)
    .where(inArray(works.authorId, perfilesDemo.map((p) => p.id)))

  return { perfiles: perfilesDemo.length, obras: obras.length }
}
